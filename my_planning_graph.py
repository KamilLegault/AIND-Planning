...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            # 1. determine what actions to add and create those PgNode_a objects
            if new_action_node.prenodes.issubset(state_nodes):
                # we found a node that has the right preconditions for the current state
                action_nodes.append(new_action_node)
                # 2. connect the nodes to the previous S literal level
                for state_node in state_nodes:
                    state_node.children.add(new_action_node)
                    # An A-level node will always have an S-level node as its parent
    ...
    def add_literal_level(self, level):
        ...
        for action_node in self.a_levels[level - 1]:
            for new_state_node in action_node.effnodes:
                state_nodes.add(new_state_node)
                new_state_node.parents.add(action_node)
                action_node.children.add(new_state_node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1 = node_a1.action
        a2 = node_a2.action
        return bool(set(a1.effect_rem) & set(a2.effect_add)) or bool(set(a1.effect_add) & set(a2.effect_rem))
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return bool(set(a1.effect_rem) & set(a2.precond_pos)) or bool(set(a1.effect_add) & set(a2.precond_neg)) or \
               bool(set(a2.effect_rem) & set(a1.precond_pos)) or bool(set(a2.effect_add) & set(a1.precond_neg))
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for prev_a1 in node_a1.parents:
            for prev_a2 in node_a2.parents:
                if prev_a1.is_mutex(prev_a2):
                    return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        for prev_s1 in node_s1.parents:
            for prev_s2 in node_s2.parents:
                if not prev_s1.is_mutex(prev_s2):
                    return False
        return True
